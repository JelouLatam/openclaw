...
### Search agents and sessions

The search box above the session list filters agents and sessions in chip mode and team mode. Press `/` to focus it when no text field has focus; Cmd/Ctrl+K still opens the command palette. Every word you type must match the agent's name, the session's title, or its custom group. Matching ignores case and accents and tolerates skipped letters, ranking prefixes, word starts, and consecutive letters first. An agent whose name matches every word lists all of its loaded sessions, and matched letters are highlighted.

With **All sessions**, matches from every agent appear in one ranked list. With **Grouped by agent**, and in chip mode, results are grouped under their agent, and chip mode searches every agent too. Results come from the sessions the sidebar already loaded and honor its status, owner, automation, and system filters. After a short pause, the longest word of two or more characters also searches the Gateway's session metadata; sessions found only there appear dimmed, with a **+N from older sessions** note below the results.

Use the arrow keys to move through results and Enter to open one: a session opens that session, and an agent opens its main chat. Escape clears the search; a second Escape leaves the box.
